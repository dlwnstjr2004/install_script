#!/bin/bash
# usage: sudo ./install_opencv_Ubuntu18_04.sh [cv_version]
# ex: sudo ./install_opencv_Ubuntu18_04.sh 4.0.1

CV_VER=4.0.1

if [ "$1" != "" ]; then
    CV_VER=$1
    echo "Change CV_VER = $CV_VER"
fi
...
mkdir ~/lib/opencv
cd ~/lib/opencv
wget -O opencv.zip https://github.com/opencv/opencv/archive/$CV_VER.zip
unzip opencv.zip
wget -O opencv_contrib.zip https://github.com/opencv/opencv_contrib/archive/$CV_VER.zip
unzip opencv_contrib.zip
cd ~/lib/opencv/opencv-$CV_VER/
mkdir build
cd ~/lib/opencv/opencv-$CV_VER/build
